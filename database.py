...
def add_tracking_id_to_queue(trackingID, sku, quantity_of_return):
  with engine.connect() as conn:
    query = text("INSERT INTO tracking_ids (tracking, SKU, refund_amount) VALUES (:tracking_id, :SKU, :refund_amount)")
    #conn.execute(query, tracking_id=trackingID)
    conn.execute(query, {"tracking_id": trackingID, "SKU":sku, "refund_amount":quantity_of_return})
